...
# ==========================================
# SDDAL_ting.sh — HRR-CL variant of SDDAL.sh
#
# Identical to SDDAL.sh except the AL loop uses ContinualTrainer.py
# (warm-start + experience replay) instead of Trainer.py (cold-start
# full retraining). The init_only block still uses Trainer.py for the
# cold-start initialization, which is correct.
#
# Key differences vs SDDAL.sh (baseline):
#   - AL loop: ContinualTrainer.py, 10 epochs, warm-start + replay
#   - Baseline: Trainer.py, 15 epochs, cold-start, full growing dataset
#   - New arg 14: buffer_size (max replay buffer entries, default 500)
#
# Usage:
#   bash SDDAL_ting.sh <beamshape> <lr> <initial_size> <fix_init?> <init_only?> <start_round> <end_round> <gpu> <scanner_batch_size> <retrain_frequency> <scan_only?> <seed> <init_seed> <buffer_size>
#
# Argument meaning:
#   fix_init:
...
#
#   init_only:
#       true  -> only initialize + train once (Trainer.py cold start), then exit
#       false -> run full SDDAL pipeline with ContinualTrainer.py
#
#   scan_only:
...
#
#   buffer_size:
#       maximum number of samples kept in the replay buffer (reservoir sampling)
#
# ==========================================
# Examples:
#
# 1) HRR-CL full pipeline  -> writes to Design_rec_1_cl/
#    bash SDDAL_ting.sh rec 0.0002 100 true false 1 200 0 5 1 false 1 123 500
#
# 2) Init-only             -> writes to Design_rec_1_cl/
#    bash SDDAL_ting.sh rec 0.0002 100 true true 1 200 0 5 1 false 1 123 500
#
# 3) Resume HRR-CL pipeline
#    bash SDDAL_ting.sh rec 0.0002 100 true false 41 200 0 5 1 false 1 123 500
#
# ==========================================
...
exp_path="Design_${beamshape}_${seed}_cl"
...
echo "========================================="
echo " SDDAL HRR-CL Pipeline (ContinualTrainer)"
...
# Uses Trainer.py (cold start) — same as baseline.
# The resulting checkpoint and init.pth.tar are shared with
# the baseline run if exp_path is the same, but here exp_path
# has the '_cl' suffix so they are kept separate.
# =========================================================
...
# =========================================================
# Timing accumulators
# Note: wall_clock_s resets to 0 at the start of each
# invocation. In resume runs it measures time elapsed
# since this invocation started, not since round 1.
# =========================================================
cumul_trainer_s=0
cumul_scanner_s=0
loop_start=$(date +%s)
...
#
# Run Trainer.py (15 epochs) on the initial set so the
# starting checkpoint is identical to the baseline's round-1
# training.  Then pre-seed the replay buffer with
# n_trained=init_size so that ContinualTrainer at round 1
# detects 0 new samples, skips retraining, and lets the
# Scanner use this checkpoint unchanged.
# =========================================================
if [ "${start_round}" -eq 1 ] && [ "${scan_only}" = false ]; then
    echo "------------------------------"
    echo "  Initial cold-start training with Trainer.py (15 epochs)"
    echo "  (matches baseline round-1 init for fair comparison)"
    echo "------------------------------"
    t0=$(date +%s)
    python3 Trainer.py \
        --train_data "${exp_path}" \
...
print(f'[buffer] n_trained={buf.n_trained}, max_size={buf.max_size} -> ${buffer_path}')
"
fi
...
do
    ...
    if [ "${scan_only}" = false ]; then
        if (( (round_sampling-1) % retrain_freq == 0 )); then
            echo "------------------------------"
            echo "  ContinualTrainer at round ${round_sampling}"
            echo "  (warm-start + replay, every ${retrain_freq} scans)"
            echo "------------------------------"
...
wall_clock_s=$(( $(date +%s) - loop_start ))
echo "========================================="
echo "   SDDAL HRR-CL Pipeline Completed Successfully!"
