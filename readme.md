...
### QuantUNetT (Scanner)
A 5-level encoder / 4-level decoder U-Net that outputs three channels: `(low, mu, high)`, representing the lower quantile, median, and upper quantile of the predicted phase. Trained with a combined quantile (pinball) + MSE loss. Used exclusively as the **scanner** — it is never used for final evaluation.
...
- **UtilityLoss**: maximizes the prediction interval `(high − low)` — selects configurations where the scanner is most uncertain.
- **MMDUniformLoss**: penalizes deviation from a uniform distribution over `[−1.5, 1.5]` via Maximum Mean Discrepancy — promotes diversity in the selected batch.
...
| 14 | `buffer_size` | *(SDDAL_ting.sh only)* Replay buffer capacity |
